...
@ApiTags('Definitions — VAT Rates')
@ApiBearerAuth()
@UseGuards(JwtAuthGuard)
@Controller('definitions/vat-rates')
export class VatRateController {
  constructor(private readonly svc: VatRateService) {}
  ...
  @Post()   create(@CurrentTenant() t: Tenant, @Body() dto: CreateVatRateDto) { return this.svc.create(t.id, dto); }
  @Patch(':id') update(@CurrentTenant() t: Tenant, @Param('id', ParseUUIDPipe) id: string, @Body() dto: UpdateVatRateDto) { return this.svc.update(t.id, id, dto); }
