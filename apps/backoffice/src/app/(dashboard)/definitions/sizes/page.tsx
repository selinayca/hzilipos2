...
interface Size { id: string; code: string; name: string; isActive: boolean; sortOrder: number; }
...
export default function SizesPage() {
  ...
  const { data = [], isLoading } = useQuery<Size[]>({
    queryKey: ['sizes'],
    queryFn: async () => (await apiClient.get('/definitions/sizes')).data.data,
  });

  const saveMutation = useMutation({
    mutationFn: (payload: object) =>
      editing ? apiClient.patch(`/definitions/sizes/${editing.id}`, payload) : apiClient.post('/definitions/sizes', payload),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['sizes'] }); close(); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.delete(`/definitions/sizes/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['sizes'] }),
  });

  function openCreate() { setEditing(null); setForm(EMPTY); setOpen(true); }
  function openEdit(item: Size) { setEditing(item); setForm({ code: item.code, name: item.name, isActive: item.isActive, sortOrder: String(item.sortOrder) }); setOpen(true); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">{t.sizes_title}</h1>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <form onSubmit={submit} className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md space-y-4">
            <h2 className="text-lg font-bold">{editing ? t.sizes_edit : t.sizes_new}</h2>
