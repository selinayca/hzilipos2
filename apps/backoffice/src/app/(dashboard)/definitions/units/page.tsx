...
interface Unit { id: string; code: string; name: string; isActive: boolean; sortOrder: number; }
...
export default function UnitsPage() {
  ...
  const { data = [], isLoading } = useQuery<Unit[]>({
    queryKey: ['units'],
    queryFn: async () => (await apiClient.get('/definitions/units')).data.data,
  });

  const saveMutation = useMutation({
    mutationFn: (payload: object) =>
      editing ? apiClient.patch(`/definitions/units/${editing.id}`, payload) : apiClient.post('/definitions/units', payload),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['units'] }); close(); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.delete(`/definitions/units/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['units'] }),
  });

  function openCreate() { setEditing(null); setForm(EMPTY); setOpen(true); }
  function openEdit(item: Unit) { setEditing(item); setForm({ code: item.code, name: item.name, isActive: item.isActive, sortOrder: String(item.sortOrder) }); setOpen(true); }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">{t.units_title}</h1>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <form onSubmit={submit} className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md space-y-4">
            <h2 className="text-lg font-bold">{editing ? t.units_edit : t.units_new}</h2>
