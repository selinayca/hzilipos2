...
interface Warehouse { id: string; code: string; name: string; address: string | null; isActive: boolean; sortOrder: number; }
interface FormState { code: string; name: string; address: string; isActive: boolean; sortOrder: string; }
const EMPTY: FormState = { code: '', name: '', address: '', isActive: true, sortOrder: '0' };
...
export default function WarehousesPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: (payload: object) =>
      editing ? apiClient.patch(`/definitions/warehouses/${editing.id}`, payload) : apiClient.post('/definitions/warehouses', payload),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ['warehouses'] }); close(); },
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiClient.delete(`/definitions/warehouses/${id}`),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['warehouses'] }),
  });

  function openCreate() { setEditing(null); setForm(EMPTY); setOpen(true); }
  function openEdit(item: Warehouse) { setEditing(item); setForm({ code: item.code, name: item.name, address: item.address ?? '', isActive: item.isActive, sortOrder: String(item.sortOrder) }); setOpen(true); }
  function close() { setOpen(false); setEditing(null); }
  function submit(e: React.FormEvent) { e.preventDefault(); saveMutation.mutate({ code: form.code, name: form.name, address: form.address || undefined, isActive: form.isActive, sortOrder: Number(form.sortOrder) }); }

  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">{t.warehouses_title}</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>{[t.code, t.name, t.warehouses_col_address, t.sort, t.status, ''].map((h) => <th key={h} className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">{h}</th>)}</tr>
          </thead>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <form onSubmit={submit} className="bg-white rounded-xl shadow-xl p-6 w-full max-w-md space-y-4">
            <h2 className="text-lg font-bold">{editing ? t.warehouses_edit : t.warehouses_new}</h2>
            ...
            <label className="block space-y-1"><span className="text-sm font-medium text-gray-700">{t.warehouses_field_address}</span><textarea value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} rows={3} className="input resize-none" /></label>
